...
import { PBTC_MARKET, USDC_MARKET } from '../helpers/BotanixConstants';

const oogaBoogaTraderAddress = Deployments.OogaBoogaAggregatorTraderV2[Network.BOTANIX].address;
const secretKey = process.env.OOGA_BOOGA_SECRET_KEY!;

describe('OogaBoogaAggregator', () => {
  const networkIdOverride = Network.BOTANIX;
  ...
  describe('#getSwapExactTokensForTokensData', () => {
    ...
    it('should work normally for full zap request', async () => {
      const subgraphUrl = process.env.BOTANIX_SUBGRAPH_URL;
      if (!subgraphUrl) {
